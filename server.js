// ═══════════════════════════════════════════════════════════════
//  CHAMPIONS FIELD — Server
//  Physics runs here. Socket.io for state sync.
//  Client-side prediction on the client.
// ═══════════════════════════════════════════════════════════════
...
const BOOST_REGEN = 0       // only pads give boost
const DASH_SPEED  = MAX_SPD * 2.0
const DASH_DUR    = 0.18
const DASH_CD     = 1.2
...
const PADS_TMPL = [
    ...
    {x:W*.3,   y:H*.3,   type:"small", value:25},
    {x:W*.7,   y:H*.3,   type:"small", value:25},
    {x:W*.3,   y:H*.7,   type:"small", value:25},
    {x:W*.7,   y:H*.7,   type:"small", value:25},
]
...
function spawnPos(team, idx, total){
    ...
    return {x:side, y:H/2-span/2+idx*step}
}
...
function repositionPlayers(room){
    const blues   = room.players.filter(p=>p.team==="blue")
    const oranges = room.players.filter(p=>p.team==="orange")
    blues.forEach((p,i)=>{ const s=spawnPos("blue",i,blues.length);     p.x=s.x;p.y=s.y })
    oranges.forEach((p,i)=>{ const s=spawnPos("orange",i,oranges.length); p.x=s.x;p.y=s.y })
}

function makeRoom(){
    return {
        players:[], ball:{x:W/2,y:H/2,vx:0,vy:0,spin:0},
        pads:PADS_TMPL.map(p=>({...p,active:true,timer:0})),
        scores:{blue:0,orange:0}, matchTime:300,
        phase:"lobby",       // lobby | kickoffCountdown | playing | goal | over
        kickoffTimer:3,
        settings:{
...
io.on("connection", socket => {
    ...
    socket.on("createRoom", (playerData) => {
        ...
        const p = makePlayer(socket.id, playerData||{})
        room.players.push(p)
        socket.emit("roomCreated", code)
        socket.emit("lobbyJoined", { myId:socket.id, players:room.players, settings:room.settings })
    })
    ...
    socket.on("joinLobby", ({ room:code, ...playerData }) => {
        ...
        socket.emit("lobbyJoined", { myId:socket.id, players:room.players, settings:room.settings, phase:room.phase })
